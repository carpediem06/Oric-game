...
inline void draw_pixel(SDL_Surface *screen, int px, int py,
		Uint32 color)
{
  int x = px;
  int y = py;
...
void plot_box(int x, int y, int w, int h, int pen)
{
...
inline void plot_pixel(int x, int y, int rgb_pen, int sdl_pen)
{
    if ((rgb_pen != vp.mask_color) || !is_mask_active(y))
    {
	draw_pixel(screen, x, y, sdl_pen);
    }
}
...
void video_init(const VideoParams *pvp)
{
	...
	screen = SDL_SetVideoMode(640,480,32,vm);
    }
    else
    {
	screen = SDL_SetVideoMode(SCREEN_WIDTH*sz,SCREEN_HEIGHT*sz,32,vm);
    }
...
static void oric_vh_render_6pixels(int x,int y, int fg, int bg,int data, int invert_flag)
{
	int i;
	int sdl_pens[2];
	int rgb_pens[2];
	...
	if (data != 0)
	{
		...
		px--;
		plot_pixel(px, y, rgb_pens[d],sdl_pens[d]);
		data = data>>1;
	    }
	}
	else
...
void video_update()
{
    ...
    hires_active = (vh_state.mode & (1<<2));
